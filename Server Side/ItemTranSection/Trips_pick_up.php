<?php
	function TripsPickUps()
	{	
		//Connecting to the Database
		$connctn = db();//Grabbing the selected customer id and setting it in SESSION
		...
		$items = $connctn->prepare("SELECT item_Frontid, inv_name, b.item_Backid, cust_id, item_modeltype, item_size
									...
									WHERE a.inv_id = b.inv_id and 
											...
											rental_status = 'Trip' and 
											c.rent_id = :a and 
											c.pick_up_date is NULL
									ORDER BY inv_name, item_modeltype, item_size");
...
?>
		...
		<head>
			...
			<legend> <?= $comments[0]['note'] ?> Trip Pick-Up </legend>
			
			<link href="//netdna.bootstrapcdn.com/bootstrap/3.0.0/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
			
		</head>
		<body style="text-align: center;">
		<button id='toggle_removing' name='toggle_removing'>Removing Items</button>
<?php
			$comment_display = "";
...
?>
						<tr>
							...
							<td> <?= $display_array[$i]['item_size'] ?> </td>
...
?>
					...
					<input type="hidden" name="item_to_be_return" id="item_to_be_return"  />  <!-- Hidden input tag keep track of which items are selected to be picked up -->
					<input type="submit" name="Checkout_trip" id="Checkout_trip" value="Checkout" /> &nbsp; <!-- Checkout Button -->
					...
					<input type="submit" name="remove_from_trip" id="remove_from_trip" value="Remove From Trip" /> &nbsp;
					<input type="hidden" name="remove_from_trip_toggle" id="remove_from_trip_toggle" value="0" />
			...
			<script type="text/javascript">
				...
				{
					$('#remove_from_trip_toggle').val('0');
					
					//Once the 'Checkout' button been clicked
					$("#Checkout_trip").click(function()
					{
						if($('#remove_from_trip_toggle').val() == '1')
						{
							alert("The 'Removing Item' toggle is on.");
							return false;
						}
						else
						{
							...
							$('#item_table').find('input:checkbox:not(:checked)').each(function () 
							{
								//Grab the value of the checkbox which is the item_Backid
								var box_value = $(this).val();
								console.log(box_value);
			...
			<script type = "text/javascript">
				...
				$(document).ready(function(){
					...
					$("#toggle_removing").click(function() 
					{
						$('input[type=checkbox]').prop('checked',false);
						$('#item_table').find('input[type="checkbox"]').each(function () {
							$(this).closest('tr').toggleClass("remove_highlight"); 
							$(this).closest('tr').removeClass("highlight");
						});
						
						if($('#remove_from_trip_toggle').val() == '0')
						{
							$('#remove_from_trip_toggle').val('1');
						}
						else
						{
							$('#remove_from_trip_toggle').val('0');
						}
						console.log('remove_from_trip_toggle = ' + $('#remove_from_trip_toggle').val());
						
					});
					
					$("#remove_from_trip").click(function() 
					{
						if($('#remove_from_trip_toggle').val() == '0')
						{
							alert("The 'Removing Item' toggle isn't on.");
							return false;
						}
						else
						{
