...
<head>
	...
	//this script goes off when a table row is clicked it checks the radio button
		$(document).ready(function(){
			$("#cart tr").click(function(){
				$(this).find('td input:radio').prop('checked',true);
				console.log("A item from the cart have been selected");
			});
			
			$("#item_selection tr").click(function(){
				$(this).find('td input:radio').prop('checked',true);
				console.log("A item from the select have been selected");
			});
		});
	</script>
	
	<!-- Little script set the item id values to the item array hidden input -->
	<script type="text/javascript">
	$(document).ready(function(){
		//Once user select an item
		$("#div_for_selection").click(function()
		{
	...
	<!-- Little script to remove item_id values from the cart array -->
	<script type="text/javascript">
	$(document).ready(function(){
		//Once user select an item on the cart to be removed
		$("#div_for_cart").click(function()
		{
			$('input[type="radio"]:checked').each(function(){
				//Grab the item_id value of the selected item
				var box_value = $(this).val();
				...
				{
					$('#cart_empty').empty();
					$(this).clone().appendTo("#cart_empty");
					$(this).remove();
				}
				else
				{
		...
		$(document).ready(function(){
			$("#cart tr").click(function()
			{
				...
				if($("#cart_empty").html() == "")
				{
					...
					tr.innerHTML = "<td colspan='4' id='delete_me'>" + "Cart Empty" + "</td>";
					tbody.appendChild(tr);
					
				}
				else
				{
					$(this).clone().appendTo("#select_empty");
					$(this).remove();
...
?>
			...
			<div id="div_for_cart" name="div_for_cart" style="float:right; width:30%;">
				...
				<table id="cart" name="cart" style="float:left">
					...
					<tbody id='cart_empty'>
						<tr>
							<td id = "hide_me"><input type="radio" id ="item_id" name="item_id[]" value = 0/></td>
							<td colspan="4" id="delete_me">
								Cart Empty
							</td>							
						</tr>
					</tbody>
			...
			<div id="div_for_package_select" name="div_for_package_select" style="border:none; float:right; margin-right:6%;">
				<fieldset id='fieldset_label' style="border:none; text-align: center;">
					<label id='header_for_table' style="font-size: 20px"> Packages: </label>
				</fieldset>
				<select name="pack" id="pack" multiple="multiple" class="pack" style="width:99%; height:15%;">
...
?>
	...
	<form method= "post" action ="<?= htmlentities($_SERVER['PHP_SELF'], ENT_QUOTES) ?>">
		<input type="submit" name="cancel" id="cancel" value="Cancel" style="float: left; margin-left:20px;"/><br />     <!-- Sends the user back to the main menu -->
	</form>
	    </fieldset>
</body>
</html>
